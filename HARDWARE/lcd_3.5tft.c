/***
   ...
   *  @description
	*
	*	实验平台：反客STM32F407VET6核心板 + 3.5寸液晶屏（型号：MCU035M1）
	*	淘宝地址：https://shop212360197.taobao.com
	*	QQ交流群：536665479
	*
	*	重要说明：
	*
	*	1. 2.6寸屏幕分辨率仅为320*240		
	...
	*
	*	4.	LCD_DrawPoint()	在指定坐标处绘制指定颜色的点
...
*	返 回 值: 无
*	函数功能: 选择要操作的寄存器
*	说    明: 由于该2.6寸的IPS屏幕比较特殊，寄存器的数值要左移一位
******************************************************************************************/
...
*	返 回 值: 无
*	函数功能: 往相应的寄存器写入数据
*	说    明: 由于该2.6寸的IPS屏幕比较特殊，要写入的寄存器数据得左移一位
******************************************************************************************/
...
*	返 回 值: 无
*	函数功能: 初始化LCD的配置
*	说    明: 主要是操作ILI9341的相关寄存器，进行必要的设置，默认竖屏显示
******************************************************************************************/
...
//while(1)
//	{
//		LCD_SetBackColor(LCD_RED);     LCD_Clear();  delay_ms(1000);
//		printf(" 写入的RGB颜色值:%x\r\n",LCD.BackColor);     //写入的RGB值
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(0,0));     //读起点坐标
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(299,21)); //读某点坐标
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(319,479)); //读终点坐标	
//		LCD_SetBackColor(LCD_GREEN);   LCD_Clear();  delay_ms(1000);
//		printf(" 写入的RGB颜色值:%x\r\n",LCD.BackColor);     //写入的RGB值
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(0,0));     //读起点坐标
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(160,233)); //读某点坐标
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(319,479)); //读终点坐标	
//		LCD_SetBackColor(LCD_BLUE);    LCD_Clear();  delay_ms(1000);
//		printf(" 写入的RGB颜色值:%x\r\n",LCD.BackColor);     //写入的RGB值
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(0,0));     //读起点坐标
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(23,161)); //读某点坐标
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(319,479)); //读终点坐标	
//		LCD_SetBackColor(LCD_GREY);    LCD_Clear();  delay_ms(1000);
//		printf(" 写入的RGB颜色值:%x\r\n",LCD.BackColor);     //写入的RGB值
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(0,0));     //读起点坐标
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(129,31)); //读某点坐标
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(319,479)); //读终点坐标	
//		LCD_SetBackColor(LCD_WHITE);   LCD_Clear();  delay_ms(1000);
//		printf(" 写入的RGB颜色值:%x\r\n",LCD.BackColor);     //写入的RGB值
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(0,0));     //读起点坐标
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(200,217)); //读某点坐标
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(319,479)); //读终点坐标	
//		LCD_SetBackColor(LCD_BLACK);   LCD_Clear();  delay_ms(1000);	
//		printf(" 写入的RGB颜色值:%x\r\n",LCD.BackColor);     //写入的RGB值
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(0,0));     //读起点坐标
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(254,120)); //读某点坐标
//		printf(" 读出的RGB颜色值:%x\r\n",LCD_RD_OnePoint(319,479)); //读终点坐标
//	}
}
...
*	说    明: 虽然反客2.6寸的IPS屏幕使用的颜色格式是16位的RGB565，但是这里为了方便用户使用，
...
void LCD_DisplayChinese(u16 x, u16 y, u8 *pText,u16 Color) 
{
   ...
   u8   disChar;	//字模的值
	...
	{
		disChar = GBK2312_1616_Table[addr + index]; //获取字符的模值
		for(counter = 0; counter < 8; counter++)
		{ 
			if(disChar & 0x01)	
			{		
			...
			{		
				LCD_DrawPoint(Xaddress,y,Color);	//否则使用背景色绘制点
			}
			disChar >>= 1;
			Xaddress++;  //水平坐标自加
			
			if( (Xaddress - x)==32 ) //如果水平坐标达到了字符宽度，则退出当前循环
			{													//进入下一行的绘制
				Xaddress = x;
				y++;
				break;
...
void LCD_DisplayText(u16 x, u16 y, u8 *pText,u16 color) 
{  
	...
	while(*pText != 0)	// 判断是否为空字符
	{
		...
		{			
			LCD_DisplayChinese(x,y,pText,color);	// 显示汉字
			x+=32;									// 水平坐标调到下一个字符处
