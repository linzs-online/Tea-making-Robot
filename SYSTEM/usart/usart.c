...
{  
   ...
   //?????RCC??  
   RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB , ENABLE); //??UART3??GPIOB???  
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10 | GPIO_Pin_11; //GPIOA9与GPIOA10
   ...
   //USART1 初始化设置
	USART_InitStructure.USART_BaudRate = bt;//波特率设置
	USART_InitStructure.USART_WordLength = USART_WordLength_8b;//字长为8位数据格式
	USART_InitStructure.USART_StopBits = USART_StopBits_1;//一个停止位
	USART_InitStructure.USART_Parity = USART_Parity_No;//无奇偶校验位
	USART_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None;//无硬件数据流控制
...
void uart5_init(u32 bound){
  ...
  GPIO_InitTypeDef GPIO_InitStructure;
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_UART5,ENABLE);//使能USART5时钟
 	...
 	GPIO_PinAFConfig(GPIOD,GPIO_PinSource2 ,GPIO_AF_UART5); //GPIOA9复用为USART1
	GPIO_PinAFConfig(GPIOC,GPIO_PinSource12,GPIO_AF_UART5); //GPIOA10复用为USART1
	//USART5端口配置TX
  ...
  USART_Init(UART5, &USART5_InitStructure); //初始化串口5
...
void USART5_IRQHandler(void)                	//串口1中断服务程序
{
	...
	{
		Res =USART_ReceiveData(UART5);//(USART5->DR);	//读取接收到的数据
